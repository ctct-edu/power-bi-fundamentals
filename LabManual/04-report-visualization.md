...
<!-- SCREENSHOT: 完成した3ページ。売上サマリー、店舗・商品分析、地域マップを示す -->
...
1. `売上サマリー`ページへ戻ります。
2. 年月スライサーの外枠を選択します。
3. 上部の［表示］→［スライサーの同期］を選択します。
4. 右側に［スライサーの同期］ペインが表示されたことを確認します。
5. ペインには`売上サマリー`、`店舗・商品分析`、`地域マップ`の3行が表示されます。
6. 3ページすべてで［同期］をオンにします。
7. 3ページすべてで［表示］もオンにします。
8. `店舗・商品分析`ページを開き、年月スライサーが表示されていることを確認して、ページ上部または下部へ移動します。
...
<!-- SCREENSHOT: スライサーの同期ペインで3ページの同期と表示をオンにした状態 -->

### 16. 同期スライサーの動作を確認する

1. `売上サマリー`ページで、年月に`2026-06`、地域に任意の1地域を選択します。
...
KPIのトレンド軸が`日付[年月開始日]`、ターゲットが`販売目標合計`であることを確認します。`売上[売上日]`や`日付[Date]`をトレンド軸にすると、最新日と月初日単位の目標が一致しません。
...
### Azure Mapsのアイコンまたは地図が表示されない

PPUライセンスがあっても、組織のテナント設定で［ユーザーはAzure Mapsビジュアルを使用できる］が無効な場合は利用できません。Power BI Desktopを更新し、講師へテナント設定とネットワーク許可を確認してください。設定変更は受講者自身では行いません。

### 都道府県が日本以外へ表示される

`店舗マスター[都道府県]`のデータカテゴリが［都道府県］または［State or Province］になっているか確認します。場所へ`店舗名`を入れず、まず`都道府県`だけで表示を確認します。
...
- [Power BIでKPIビジュアルを作成する](https://learn.microsoft.com/ja-jp/power-bi/visuals/power-bi-visualization-kpi)
...
- [Azure Mapsで地理情報を設定する](https://learn.microsoft.com/ja-jp/azure/azure-maps/power-bi-visual-geocode)
- [Power BIレポートでドリルスルーを設定する](https://learn.microsoft.com/ja-jp/power-bi/create-reports/desktop-drillthrough)

...
| 1側 | 1側の列 | 多側 | 多側の列 | カーディナリティ | 方向 | アクティブ |
|---|---|---|---|---|---|---|
| 商品カテゴリ | 商品カテゴリ | 商品マスター | 商品カテゴリ | 一対多 | 単一 | オン |
...
| 粗利率 | 売上 | `粗利率 = DIVIDE([粗利益], [売上合計])` | パーセント・小数1桁 | 安全な除算 |
| 販売目標合計 | 販売目標 | `販売目標合計 = SUM('販売目標'[販売目標金額])` | 整数・桁区切り | 別テーブルの集計 |
| 目標達成率 | 売上 | `目標達成率 = DIVIDE([売上合計], [販売目標合計])` | パーセント・小数1桁 | 複数テーブルの比較 |
| 前月売上 | 売上 | `前月売上 = CALCULATE([売上合計], DATEADD('日付'[Date], -1, MONTH))` | 整数・桁区切り | 時間インテリジェンス |
...
時間に余裕がある場合は、手順13と同じ共通操作で次の3つを作成します。式は資料に残しますが、演習03の必須完了条件には含めません。
